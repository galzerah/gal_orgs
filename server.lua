...
src = {}
Tunnel.bindInterface("gal_orgs",src)
Proxy.addInterface("gal_orgs",src)
local inAdmin = {}

vCLIENT = Tunnel.getInterface("gal_orgs")

vRP.prepare("gal_orgs/getOrg", "SELECT * FROM gal_orgs WHERE org = @org")
vRP.prepare("gal_orgs/updateMembers", "UPDATE gal_orgs SET membros = @membros WHERE org = @org")
vRP.prepare("gal_orgs/updateOffice", "UPDATE gal_orgs SET groupsOffice = @groupsOffice WHERE org = @org")
...
function src.leaveOrg(myOrg)
    ...
    if rows[1] then
        ...
        vRP.execute("gal_orgs/updateMembers", { org = myOrg, membros = tostring(json.encode(members)) })
    end
end
            ...
            if #rows > 0 then
                ...
                local request = vRP.request(nsource, "Você está sendo convidado para a <b>"..org.."</b> deseja aceitar?", 30)
                    ...
                    if request then
                        ...
                        vRP.execute("gal_orgs/updateMembers", { org = org, membros = json.encode(members) })
                    end
                else
    ...
    if rows[1] then
        ...
        if action == 'withdraw' then
            if hasPermissionWithdraw(user_id, myOrg) then
                ...
                vRP.execute("gal_orgs/updateBanco", { org = myOrg, banco = bankValue, bancoHistorico = json.encode(bankHistory)})
                local rows2 = vRP.query("gal_orgs/getOrg", { org = myOrg })
...
function src.updateOffice(index, formatOffices)
    ...
    if hasPermissionLeader(user_id, myOrg) then
        ...
        vRP.execute("gal_orgs/updateOffice", { org = myOrg, groupsOffice = tostring(json.encode(offices)) })
    end
end
...
function src.reward(data)
    ...
    if not playerRewarded then
        if tonumber(playerDailyFarm) >= tonumber(GoalsMaxDaily) then
            ...
            vRP.execute("gal_orgs/updateRewards", {org = myOrg, rewards = json.encode(rewards)})
...
function src.registerRewards(user_id, item, amount)
    ...
    vRP.execute("gal_orgs/updateRewards", {org = myOrg, rewards = json.encode(rowRewards)})
end

CreateThread(function()
    vRP.execute("gal_orgs/initTable", {})
    
    if orgsConfig.main.createAutomatic then
        for k,v in pairs(orgsConfig.List) do
            vRP.execute("gal_orgs/initGroups", { org = k, maxMembros = v.config.maxMembers})
        end
    end
    ...
    if orgsConfig.main.atualizeOfficesAutomatic then
        ...
        local function verificaPrefix(org, prefix)
            local rowsOrg = vRP.execute("gal_orgs/getOrg", {org = org})
        ...
        for org, novosGrupos in pairs(novosGruposPorOrg) do
            vRP.execute("gal_orgs/updateOffice", {groupsOffice = json.encode(novosGrupos), org = org})
        end
    end
    ...
    if orgsConfig.main.atualizeRewardsInfoGoalAutomatic then
        for org, orgData in pairs(orgsConfig.List) do
            ...
            vRP.execute("gal_orgs/updateRewards", {rewards = json.encode(rewardsList), org = org})
        end
    end
    ...
    if user_id then
        ...
        if myOrg then
            ...
            if #rows > 0 then
                ...
                vRP.execute("gal_orgs/updateMembers", { org = myOrg, membros = json.encode(members) })
            end
        end
    end
end)