    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            if (detect_data != 'registration' && (detect_data != 'login')) {
                ...
                $.post("http://gal_orgs/onClose", JSON.stringify({
                    close: data_atual
                }));
            }
        }
...
function requestOrgs() {
  $.post("https://gal_orgs/requestOrgs", JSON.stringify({}), (data) => {
...
function sendName() {
  $.post(
    "http://gal_orgs/requestPlayerName",
    JSON.stringify({
...
function openPromove(name) {
  $.post(
    "http://gal_orgs/requestPromovePlayer",
    JSON.stringify({
...
function requestBank() {
  $.post("https://gal_orgs/requestBank", JSON.stringify({}), (data) => {
...
function selectOffices(officeProp, indexProp, attr) {
  ...
  $.post(
    "https://gal_orgs/requestSelectOffice",
    JSON.stringify({
      index: officeProp,
    }),
    (data) => {
...
function setConfigType(type) {
  fetch('http://gal_orgs/setConfigType', {
      method: 'POST',
      body: JSON.stringify({
        type
      })
...
async function requestConfigDatas() {
  await fetch('http://gal_orgs/requestConfigDatas')
    .then(res => res.json())
    .then(data => {
      inputValue = new Date().getDate();
      loadConfigData(data)
...
function reward() {
  fetch('http://gal_orgs/reward', {
    method: 'POST',
    body: JSON.stringify({
      currentType
    })
  })
  fetch('http://gal_orgs/onClose', {
    method: 'POST',
    body: JSON.stringify({
      close: 'orgs'
    })
...
function requestOffices() {
  $.post('https://gal_orgs/requestOffices', JSON.stringify({}), (data) => {
    $('.patents_orgs').html('');
    $('.edit_patents_orgs').html('');
